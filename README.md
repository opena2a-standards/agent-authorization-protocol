...
The Internet-Draft revision that carries the `0.5.0-draft` text is `draft-fane-opena2a-aap-02`
(source and rendered text in this repository; not yet submitted). Authored in the open; intended
for submission as an IETF Internet-Draft. Each document carries its own version at the top;
`CHANGELOG.md` records what moved between drafts and which draft revision pairs with which
version.
