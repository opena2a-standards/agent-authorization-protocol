...
## A Standard for Resolving Agent Trust Into Resource Access Without Exposing Credentials

**Version:** 0.1.0-draft
**Authors:** OpenA2A
**Date:** June 2026
**Status:** Draft. Intended for submission as an IETF Internet-Draft.
...
Trust eXtension (ATX) is the signed credential that carries that trust. The **Agent Authorization
Protocol (AAP)** defined here is the authorization layer of the ATP family: it resolves an agent's
trust into concrete, scoped access to a real downstream resource — **without the credential value
ever entering the agent's reasoning context, and therefore without it ever reaching a language
model provider.**
